...
define netplan::vlans (
  ...
  if $gateway4 != undef {
      notify {"puppet-netplan vlan ${name}: gateway4 has been deprecated, use default routes instead.": }
  }
  if $gateway6 != undef {
      notify {"puppet-netplan vlan ${name}: gateway6 has been deprecated, use default routes instead.": }
  }
